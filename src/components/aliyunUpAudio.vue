<template>
	<div class="up-video-box" style="margin-top:20px;">
		<p class="m-img-text-title" style="margin-top: 3px;"><span>*</span> 音频</p>
		<div class="up-video-operation">
			<a class="video-name-show" v-if="fileName!=''">{{fileName}} <span v-show="!isProgress" class="up-cancel" @click.stop.prevent="cancelUpload"></span></a>
		...
		<p class="up-video-tip">支持格式：请上传m4a、mp3格式文件</p>
	</div>
</template>
<script>
	...
	export default {
		...
		methods:{
			 ...
			 */
			addFile(event){
				if(event.target.files.length > 0 && event.target.files[0].size > 1024*1024*1024){
					this.errText = '文件最大为1G'
					return ;
				}
				var file = event.target.files[0];
				var supportFormat = '.m4a、.mp3';
				var flag = false;
            ...
            getVoucher(fileName,fn){
                api.getUploadAuthFn(qs.stringify({
                    name:fileName,
                    title:"呱呱财经"
                }))
                .then(response => {
