<template>
	<div class="img-text-wrap">
		<div class="img-select-big-box" style="margin-top:20px;">
	    	<p class="m-img-text-title"><span>*</span>专辑封面：</p>
	    	<div class="select-img-warp" style="margin-left:19px;">
	    		...
	    		<p class="up-img-tip">请上传正方形图片，否则显示会有变形。支持上传JPG、JPEG、PNG文件,图片尺寸需大于400像素，<br>文件大小在500kb以内，最大支持1MB以内</p>	
	    	</div>
	    </div>
		<div class="video-title-box">
			<p class="m-img-text-title" style="margin-top: 12px;"><span>*</span>专辑标题：</p>
		...
		<div class="btn-list">
				...
				<p class="up-submit" :class="{yes: isYes}" @click="saveData">提交审核</p>
			</div>
	    <layer></layer>
    </div>
</template>
...
export default {
	...
	computed:{
		isYes(){
			if(this.dataObj.uploadType == 2){
				if(this.imgDataUrl != '' && this.videoTitle != '' && this.videoRemark != '' && this.defaultImgsSelect.length > 0 && this.videoRemark != ''){
					return true;
				}else{
					return false;
				}
			}else{
				if(this.imgDataUrl != '' && this.videoTitle != '' && this.videoRemark != '' && this.defaultImgsSelect.length > 0 && this.dataObj.typeId && this.dataObj.typeTwoId && this.videoRemark != ''){
					return true;
				}else{
					return false;
				}
	...
	methods:{
		//请求标签集合
			reloadLbel(){
				api.getdataOneFn({typeId:this.dataObj.typeId})
					.then(response=>{
			...
			getSelectImgData(){
				if(this.dataObj.uploadType == 2){
					api.rePublishUpAlbumFn(
					qs.stringify({//重新上传专辑接口
                        albumId:this.dataObj.albumId,
                        typeOneId:this.dataObj.typeId,
                        typeTwoId:this.dataObj.typeTwoId,
                        albumName:this.videoTitle,
                        albumPic:this.imgDataUrl,
                        albumDesc:this.videoRemark,
					...
					api.saveAlbumUrlFn(qs.stringify({
						typeOneId:this.dataObj.typeId,
                        typeTwoId:this.dataObj.typeTwoId,
                        albumName:this.videoTitle,
                        albumPic:this.imgDataUrl,
			...
			cropUploadSuccess(jsonData, field){
				...
				this.imgDataUrl = jsonData.content
				if(jsonData.state == 0) {
