<template>
    <div>
        ...
        <div class="zjxq_main">
            <dl id="xq_main">
              <dt><img :src="$fun.getUrlVerify(titleDate.albumPic)" alt="专辑封面" /></dt>
              <dd>
                <p class="title_gp">{{titleDate.albumName}}
                    <span v-for="(item,index) in titleDate.labelList" :key="index">{{item.labelName}}</span></p>
                <p><em>分类：</em><var>{{titleDate.typeOneName}}-{{titleDate.typeTwoName}}</var></p>
                <p><em>简介：</em><var>{{titleDate.albumDesc}}</var></p>
                <a href="javascript:;" class="scyp_btn" @click="$router.push({path:'/uploadVideo'})">上传视频</a>
              </dd>
            </dl>
            <div v-if="type == 1">
                ...
                <div class="jianjie"><span>图片简介：</span>
                    <a target='_blank' :href='$fun.getUrlVerify(titleDate.coursePic)' v-if="titleDate.coursePic!=''">
                        <img :src="$fun.getUrlVerify(titleDate.coursePic)" />
                    </a>
                </div>
            </div>
            <div class="jianjie"><span>专辑默认显示顺序：</span>
                <label><input name="sort" type="radio" :checked="checkedOne" value="0" @click="videoSort(0)" />正序显示</label>
                <label style="margin-left:10px;"><input name="sort" type="radio" :checked="checkedTwo" value="1" @click="videoSort(1)"/>倒序显示</label>
            </div>
        </div>
        <div class="main-table-warp" style="width:920px;">
            <v-table
                ...
                :row-height="80"
                :title-row-height="50"
                :title-click="clickDetail"
                :select-change='selectRow'
                :select-all='selectAll'
                :row-click="rowClick"
                @on-custom-comp="customCompFunc"
            ></v-table>
            <p v-if="type == 0" class="move transforAlbum" @click="transferAlbum"><span>转移至其他专辑</span></p>
            ...
            <div class="tip">
                <div class="tip-content">
                    <div class="tip-content-msg-one">
                        <div class="boxText">已选择{{checkoutList.length}}个视频</div>
            ...
            <div class="tip">
                <div class="tip-content">
                    <div class="tip-content-msg">
                      <p class="tip-content-video-info">{{titleDate.albumName}}:{{curVideoTitle}}</p>
                        <div class="boxText" style="font-size:14px; mar-left:45px; text-indent:24px;">{{curSortModTxt}}</div>
                        <div class="boxText"><span>排序：</span>
                          <input type="number" placeholder="请输入大于0的整数" name="" v-model="szpx" @propertychange="zhihui()" @input="zhihui()" class="szpx" onkeyup="value=parseInt(value.replace(/^[0][\*]*/g,''))">
                        </div>
                    </div>
                    <ul class="content-btn-list">
                      <li>
...
<script>
    ...
    import {AIDEO_PLAY,AUDIT_RRESULT,UPLOAD_MODIFY_FILE} from '@/utils/constant'//类型
    ...
    export default{
        ...
        data() {
            return {
                ...
                checkoutList:[],
                columns: [                    
                    {field: 'num', title:'序号', width: 90, titleAlign: 'center',columnAlign:'center',isResize:true},
                    {field: 'videoTitle', title:'视频标题', width: 167, titleAlign: 'center',columnAlign:'center',isResize:true},
                    {field: 'videoPic', title: '视频封面', width: 100, titleAlign: 'center',columnAlign:'center',isResize:true,
                        formatter:(rowData,rowIndex,pagingIndex,field)=>{
                                return "<a target='_blank' href='"+rowData.videoPic+"' ><img style='width:80px;height:80px' src='"+rowData.videoPic+"'></a>"
                    }},
                    {field: 'videoRemark', title: '视频简介', width: 182, titleAlign: 'center',columnAlign:'center',isResize:true},
                    {field: 'view', title:'播放量', width: 72, titleAlign: 'center',columnAlign:'center',isResize:true},
                    {field: 'shelfTime', title:'上架时间', width: 167, titleAlign: 'center',columnAlign:'center',isResize:true},
                    ...
                    {field: 'custome-adv', title: '操作',width: 150, titleAlign: 'center',columnAlign:'center',componentName:"tableOpa",isResize:true}
                ],
                tipData:null,
                downVideoId:'',
                albumId:'',
                ...
                transferFlag2:false,
                szpx:'',//当前选中视频的排序
                curVideoTitle: '',//当前选中视频标题
                curSortModTxt: '',//当前排序方式文字提示
                albumslotId:'',
                sortNum:'',
                checkedOne:'checked',
                checkedTwo:''
        ...
        methods:{
            ...
            rowClick(rowIndex,rowData,column){
             if(column.field=="sort"){
                this.albumslotId = rowData.vid
                this.transferFlag2 = true
                this.szpx = rowData.sort;
                this.curVideoTitle = rowData.videoTitle;
              }
            },
            //取消排序
            transFerFalsePx(){
            ...
            transFerTruePx(){
              if(this.szpx == '' || this.szpx == 0){
                ...
                api.videoSzpxFn({vid:this.albumslotId,sort:this.szpx})
                  .then(response => {
            ...
            videoSort(sort){
                api.videoSortFn({
                    sort:sort,//传参
                    albumId:this.titleDate.albumId
                })
                .then(response=>{
                    if(response.status == 200){
            ...
            transFerTrue(){
                if(this.albumIdTwo != ''){
                    ...
                    this.$store.commit('layer/setSuccessCall',function(){
                        api.transferAlbumFn(qs.stringify({vids:this.checkoutList.join(','),albumId:this.albumIdTwo}))//转移接口
                        .then(response => {
                            if(response.status == 200){
                                this.checkoutList = []
                                this.albumIdTwo = ''
            ...
            selectRow(selection,rowData){
                this.checkoutList = []
                selection.map(item=>{
                    this.checkoutList.push(item.vid)
                })
            },
            //全选
            selectAll(selection){
            ...
            customCompFunc(params){
                if (params.type[0] === 'edit'){   //编辑后上传
                    this.$router.push({path:'/uploadVideo',
                        query:{
                            uploadType:UPLOAD_MODIFY_FILE,
                            ...
                            albumName:this.titleDate.albumName,
                            model:params.rowData['model'],//音频类型0免费1付费
                            sort:params.rowData['sort']
                        }})
                }else if (params.type[0] === 'discharge'){   //下架
                   this.$store.commit('layer/updateTipData',{
                        showbox:'pattern-1',
                    ...
                    this.$store.commit(
                      ...
                      function() {
                        api
                          .courseFn({vid: this.downVideoId,model:2}) //取消试听课
                          .then(response => {
                            if (response.status == 200) {
                              var data = response.data;
                              if (data.state == 0) {
                    ...
                    this.$store.commit(
                      ...
                      function() {
                        api
                          .courseFn({vid:this.downVideoId,model:1}) //设置试听课
                          .then(response => {
                            if (response.status == 200) {
                              var data = response.data;
                              if (data.state == 0) {
            ...
            getDate(){
                //console.log(this.$route.query.albumId+"11111111111");
                api.seeAlbumFn({//视频专辑列表接口
                    pageSize:20,
                ...
                .then(response=>{
                    if(response.status == 200){
                        var data = response.data;
                        if(data.state == 0){
                            this.tableData = data.content.videos.list;
                            this.tableData.map(item=>{
                                item.videoPic = this.$fun.getUrlVerify(item.videoPic)
                            })
                            this.sortNum = data.content.albumInfo.sort;
                              if(this.sortNum == 1){
                            ...
                            if(this.activePage == 1){
                                this.titleDate = data.content.albumInfo
                            }
                            this.pageNum = data.content.videos.pages;
        ...
        created(){
            ...
            this.$store.dispatch('tableOpa/updateOpaList',[
                {content:'下架',style:{'color':'#4d9dfc'},type:['discharge',this.type],permissFlag:'details'},
                {content:'编辑',style:{'color':'#4d9dfc'},type:['edit',this.type],permissFlag:'details'},  
                {content:'取消试听课',style:{'color':'#4d9dfc'},type:['qxcourse',this.type],permissFlag:'details'},
                {content:'设置试听课',style:{'color':'#4d9dfc'},type:['szcourse',this.type],permissFlag:'details'}  
            ])
            //获取专辑列表(转移)
            api.zhuanyiFn()
            .then(response=>{
...
.move{overflow:hidden; margin-top:10px; height:60px;}
.transforAlbum{display: inline-block}
.move span{padding:8px; border:1px solid #d2d2d2; border-radius:5px; color:#333333; font-size:14px; display:block; width:120px; cursor:pointer;}
.zjxq_main{width:905px; overflow:hidden; margin:0 auto; padding-top:20px;}
.zjxq_main dl{overflow:hidden; margin-bottom:20px;}
.zjxq_main dl dt{width:120px; height:120px; float:left;}
.zjxq_main dl dt img{width:120px; height:120px; display:block;}
.zjxq_main dl dd{float:left; margin-left:17px; width:758px; position:relative;}
.zjxq_main dl dd p{color:#333333; font-size:14px;clear:both;}
.zjxq_main dl dd p em{display:block; float:left;}
.zjxq_main dl dd p var{display:block; float:left; font-style:normal; width:405px; padding-bottom:15px;}
.zjxq_main dl dd p.title_gp,.zjxq_main dl dd p em{color:#333333; font-size:18px; padding-bottom:15px; font-weight:bold; font-style:normal;}
.title_gp span{
    width:68px; 
    height:24px; 
    display:inline-block; 
...
.go-back-btn{
    ...
    .cleaars{clear:both; width:100%; margin-top:15px;}
