		...
		<div class="fill-content">
			<div class="video-classify-show-box" v-if="dataObj.uploadType != 2 && lecturerAuth">
				...
				<div class="select-warp2" style="margin-top:10px;">
                    <label><input type="radio"  name="mf" value="1" checked @click="checkType(1)"/>&nbsp;&nbsp;免费专辑</label>
                    <label style="margin-left:15px;"><input type="radio"  name="mf" value="2" @click="checkType(2)"/>&nbsp;&nbsp;付费专辑</label>
                </div>
			</div>
			<div v-if="mian_fu==2">
			...
			<div class="img-text-wrap">
				<div class="img-select-big-box" style="margin-top:20px;">
			    	...
			    	<div style="float:left;"><up-image-all :imagesType="imagesType" :imgUrl="imgUrl2" size="1024" @callbackImg="(url)=>{callbackImg(2,url)}"></up-image-all></div>
			...
			<div class="img-select-big-box">
	    		...
	    		<ul class="video-screenshot" style="padding: 0px 18px 0 18px;">
						<li class="selectLi" v-for="(item,key) in defaultImgs" :class="{active:defaultImgsSelect.indexOf(item.id) != -1}" @click="selectImg(3,key,item)" :key="key"><button type="button" class="selectBtn">{{item.name}}</button><span class="select-icon"></span></li>
				</ul>
		</div>
...
<script>
	...
	export default {
		...
		computed:{
			...
			if(this.mian_fu == 2){//付费
				if(this.dataObj.uploadType == 2){//编辑this.keRemark !='' && this.teacherRemark !='' && this.imgUrl1!='' && 
					...
					if(this.imgUrl2 != '' && this.albumName != '' && this.videoRemark != '' && this.defaultImgsSelect.length > 0 && this.typeTwoId ){
						return true;
					}else{
						//console.log(2)
						return false;
		...
		methods:{
			...
			//请求标签集合
			reloadLbel(){
				api.audioalbumlabelFn({typeId:this.typeOneId})
					.then(response=>{
			...
			getSelectImgData(){
				...
				if(this.dataObj.uploadType == 2){//编辑
					api.audioalbumeditsaveFn(qs.stringify(Object.assign({//重新上传专辑接口(编辑)
                        id:this.dataObj.albumId,//专辑id
                        typeOne:this.dataObj.typeOneId,//一级分类
                        typeTwo:this.dataObj.typeTwoId,//二级分类
                        title:this.albumName,//专辑名称
                        pic:this.imgUrl2,//封面
                        referral:this.videoRemark,//专辑简介
                        model:this.mian_fu,//专辑类型（1免费2付费）
					...
					api.audioalbumeditsaveFn(qs.stringify(Object.assign({
						typeOne:this.typeOneId,//一级分类
                        typeTwo:this.typeTwoId,//二级分类
                        title:this.albumName,//专辑名称
                        pic:this.imgUrl2,//封面
                        referral:this.videoRemark,//专辑简介
                        model:this.mian_fu,//专辑类型（1免费2付费）
			...
			selectImg(boxNumber, index, item){
				if(this.defaultImgsSelect.indexOf(item.id) == -1){
					...
					this.defaultImgsSelect.splice(this.defaultImgsSelect.indexOf(item.id),1)
		...
		created(){
			...
			if(this.$route.query.uploadType == 2){
				...
				//this.albumId=this.dataObj.albumId//专辑id
				this.typeOneId = this.dataObj.typeOneId
				this.typeTwoId = this.dataObj.typeTwoId
				...
				api.audiogetAuthFn().then(res=>{//获取是否拥有创建付费专辑权限
					if(res.status ==200){
						var data = res.data
						if(data.state == 0){
							if(data.content == 2){
