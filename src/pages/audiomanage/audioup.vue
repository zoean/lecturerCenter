            ...
            <div class="condition-video-online-warp">
                ...
                <div class="select-warp">
                    ...
                    <select v-model="selectedRank">
                        ...
                        <option value="2">观看数从高到低</option>
                        <option value="1">观看数从低到高</option>
                    </select>
                </div>
                <div class="select-warp">
        ...
        data() {
            return {
                ...
                columns: [
                    ...
                    {field: 'playNum', title: '播放量', width: 80, titleAlign: 'center',columnAlign:'center',isResize:true},
                    {field: 'updateTime', title: '上架时间',width: 150, titleAlign: 'center',columnAlign:'center',isResize:true},
                    {field: 'custome-adv', title: '操作',width: 100, titleAlign: 'center',columnAlign:'center',componentName:'tableOpa',isResize:true},
                ],
                downDateIndex:0,
            }
        },
        	...
        	customCompFunc(params){
                if (params.type === 'discharge'){   //下架
                    ...
                    this.$store.commit('layer/setSuccessCall',function(){
                        api.downAudioFn({audioId:this.downAudioId})
                        .then(response => {
                            if(response.status == 200){
                                var data = response.data;
                                if(data.state == 0){
                    ...
                    this.downAudioId = params.rowData['id'];
                    this.$store.dispatch('layer/show');
            ...
            api.dateAudioListFn({
                ...
                .then(response=>{
                    if(response.status == 200){
                        ...
                        if(data.state == 0){
                            this.tableData = data.content.list;
                            this.$store.dispatch('navNum/audioUp',data.content.total)
                        }else{
                            console.log(data.message);
                        }
                    }else{
